...
selector= selector.fit(pd.DataFrame(scaler.fit_transform(imputer.fit_transform(X[train_index, :]))
                                    , columns=feature_names_all).fillna(0), y[train_index])
