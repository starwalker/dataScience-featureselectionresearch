...
print(F'arg n_features to select: {args.n}')
MAX_MEMORY = "32g"
...
obs = IsObserved().fit(df.iloc[train_index, :].loc[:, feature_names_all] )
imputer = SimpleImputer().fit(df.iloc[train_index, :].loc[:, feature_names_all] )

transformers = [('obs', obs, feature_names_all), ('imp', imputer, feature_names_all)]
pipe = ColumnTransformer(transformers).fit(df.iloc[train_index, :].loc[:, feature_names_all] )
...
feature_names_transformed_all = np.array(obs.get_feature_names() + list(feature_names_all))

n_features_to_select= np.min([X_test.shape[1], args.n])

print(F'running RFE to select: {n_features_to_select} from {X_train.shape[1]}....')
...
test_preds = model.predict_proba(pd.DataFrame(X_test,columns=feature_names_transformed_all).iloc[:, support_index]).iloc[:, 1]
test_pred_labels = model.predict(pd.DataFrame(X_test,columns=feature_names_transformed_all).iloc[:, support_index])

## predicts the training data
train_preds = model.predict_proba(pd.DataFrame(X_train,columns=feature_names_transformed_all).iloc[:, support_index]).iloc[:, 1]
train_pred_labels = model.predict(pd.DataFrame(X_train,columns=feature_names_transformed_all).iloc[:, support_index])
...
run_name = 'evalML_rfe_ w_isObeserved'
